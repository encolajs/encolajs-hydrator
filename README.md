...
Additionally, the library provides 2 base classes for easy extension: **BaseModel** and **BaseCollection**.
...
### BaseModel and BaseCollection

**BaseModel** is a base class that can be used to create domain models with automatic type casting and serialization.

**BaseCollection** is a base class for managing collections of BaseModel instances. Collections are array-like objects that contain only instances of specific class. Any item added to the collection is cast to the specified class.

```javascript
let Person = builder.createModelClass({
  name: 'string',
  age: 'number',
  birthdate: 'date',
  active: 'boolean'
...
let People = builder.createCollectionClass(Person)

castingManager.registerModel('Person', Person, People)

let people = castingManager.cast([
  { name: 'John Doe', age: '30', birthdate: '1993-05-15', active: 1 },
  { name: 'Jane Smith', age: '25', birthdate: '1998-02-20', active: 0 }
], 'Person_Collection') // `Person_Collection` is the key used to register the casting function

people[0] instanceof Person // true
people[0].greet() // "Hello, my name is John Doe"
```

The **BaseCollection** class comes with a number of useful methods for working with collections:

```javascript
activePeople = people.filterBy('active') // evaluate the truthiness of the `active` property
john = people.findBy('name', 'John Doe') // returns the first item that matches the criteria

peopleByAge = people.sortBy('age', 'desc') // sort the collection by the `age` property
```
...
// Set up User class
let User = builder.createModelClass({
  id: 'number',
  name: 'string',
  email: 'string',
  role: 'string'
...
// Set up Post class
let Post = builder.createModelclass({
  id: 'number',
  title: 'string',
  content: 'string',
  author_id: 'number',
...
let Comment = builder.createModelClass({
  id: 'number',
  post_id: 'number',
  author_id: 'number',
  author: 'User',
...
castingManager.registerModel('Comment', Comment, builder.createCollectionClass(Comment))
